  ...
  function ajaxCall(url, success, error) {
    ...
    if ($) {
      ...
      xhr.onload = function () {
        requestComplete();
        // PATCH_BEGIN
        var statusText = xhr.getResponseHeader("X-Status-Reason") || xhr.statusText;
        switch (xhr.status) {
          case 200:
            success(JSON.parse(xhr.responseText), statusText, xhr);
            break;
          case 413:
            error(xhr, "error", statusText || "Request Entity Too Large");
            break;
          default:
            error(xhr, "error", statusText);
        }
        // PATCH_END
      };
      xhr.send();
